...
- **Diff rendering** — repaints only when the line or terminal size changes, so a
...
```sh
alias lyrics='lrc-vis --lrc-dir ~/.local/share/lrc-tools/lyrics/processed'
alias lyrics-word='lrc-vis --lrc-dir ~/.local/share/lrc-tools/lyrics/processed --wlrc'
```
